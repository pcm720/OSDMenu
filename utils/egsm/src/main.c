...
struct gsm_state {
  ...
  uint32_t SetGsCrt : 1;
  uint32_t HalfHeight : 1;
  uint32_t LineDouble : 1;
  uint32_t VSINTCount : 4;
  uint32_t VSINTPrev : 1;
  uint32_t VCKDiv : 2; // VCK Divide (1 or 2)
  uint32_t Spare : 22;
...
/*
 * Divide x/width by 2 if possible
 *
...
static uint64_t mod_DISPLAY(struct gsm_state *pstate, uint64_t r) {
  uint32_t dx, dy, magh, magv, dw, dh;
  uint32_t magh_new, magv_new, dw_new;
...
void el2_c_handler(ee_registers_t *regs) {
  ...
  case (uint32_t)GS_REG_CSR:
    ...
    if (pstate->flags & EGSM_FLAG_C_1) {
    ...
    } else if (pstate->flags & EGSM_FLAG_C_2) {
      //
      ...
      FIELD_emu = pstate->VSINTCount & 1;
    } else if (pstate->flags & EGSM_FLAG_C_3) {
      //
...
static void hook_SetGsCrt(short int interlace, short int mode, short int ffmd) {
  ...
  if (ffmd == 1) {
    ...
    if (pstate->flags & EGSM_FLAG_FRM_FP1 /*&& interlace*/) {
      // Force 240p/288p
      pstate->VCKDiv = 1; // Divide by 2
      pstate->HalfHeight = 1;
      pstate->LineDouble = 0;
      interlace = 0;
      ...
      ffmd = 1;
    } else if (pstate->flags & EGSM_FLAG_FRM_FP2) {
      // 576p not supported on pre-DECKARD consoles
      if (mode == 3 && (pstate->flags & EGSM_FLAG_NO_576P))
        goto no_gsm;

      // Force line-doubling to 480p/576p
      pstate->VCKDiv = 2; // Divide by 2
      pstate->LineDouble = 1;
      interlace = 0;
      if (mode == 2)
        mode = 0x50; // 480p
    ...
    if (pstate->flags & EGSM_FLAG_FLD_FP) {
      // 576p not supported on pre-DECKARD consoles
      if (mode == 3 && (pstate->flags & EGSM_FLAG_NO_576P))
        goto no_gsm;
  ...
  if (pstate->flags & (EGSM_FLAG_C_1 | EGSM_FLAG_C_2 | EGSM_FLAG_C_3)) {
...
int main(int argc, char *argv[]) {
  ...
  if (state.flags & (EGSM_FLAG_C_1 | EGSM_FLAG_C_2 | EGSM_FLAG_C_3)) {
