...
#define BANNER "ATA device driver for HDD OSD %s - Copyright (c) 2003 Marcus R. Brown\n"
#define VERSION "v1.1"
...
static ata_cmd_info_t ata_cmd_table[] = {{0, 1},    {3, 1},    {8, 5},    {0x20, 2}, {0x30, 3}, {0x32, 8}, {0x38, 3}, {0x40, 1}, {0x70, 1}, {0x87, 2},
                                         {0x8e, 7}, {0x90, 6}, {0x91, 1}, {0x92, 3}, {0xa1, 2}, {0xb0, 7}, {0xc0, 1}, {0xc4, 2}, {0xc5, 3}, {0xc6, 1},
                                         {0xc8, 4}, {0xca, 4}, {0xcd, 3}, {0xda, 1}, {0xde, 1}, {0xdf, 1}, {0xe0, 1}, {0xe1, 1}, {0xe2, 1}, {0xe3, 1},
                                         {0xe4, 2}, {0xe5, 1}, {0xe6, 1}, {0xe7, 1}, {0xe8, 3}, {0xec, 2}, {0xed, 1}, {0xef, 1}, {0xf1, 3}, {0xf2, 3},
                                         {0xf3, 1}, {0xf4, 3}, {0xf5, 1}, {0xf6, 3}, {0xf8, 1}, {0xf9, 1}, {0x25, 4}, {0x35, 4}, {0xea, 1}};
#define ATA_CMD_TABLE_SIZE (sizeof ata_cmd_table / sizeof(ata_cmd_info_t))

static ata_cmd_info_t sec_ctrl_cmd_table[] = {{0xec, 2}, {0xf3, 1}, {0xf4, 1}, {0xf5, 1}, {0xf1, 3}, {0xf2, 3}, {0x30, 3}, {0x20, 2}};
#define SEC_CTRL_CMD_TABLE_SIZE (sizeof sec_ctrl_cmd_table / sizeof(ata_cmd_info_t))

static ata_cmd_info_t smart_cmd_table[] = {{0xd0, 2}, {0xd2, 1}, {0xd3, 1}, {0xd4, 1}, {0xd5, 2}, {0xd6, 3}, {0xd8, 1}, {0xd9, 1}, {0xda, 1}};
...
int ata_io_finish() {
  ...
  if (type == 1 || type == 6) { /* Non-data commands.  */
    ...
    if ((res = ata_dma_complete(cmd_state->buf, cmd_state->blkcount, cmd_state->dir)) < 0)
      goto finish;
...
int ata_device_flush_cache(int device) {
  ...
  if (lba_48bit[device]) {
    res = ata_io_start(NULL, 1, 0, 0, 0, 0, 0, (device << 4) & 0xffff, ATA_C_FLUSH_CACHE_EXT);
  } else {
    res = ata_io_start(NULL, 1, 0, 0, 0, 0, 0, (device << 4) & 0xffff, ATA_C_FLUSH_CACHE);
  }
  if (!res)
...
int ata_device_sce_security_init(int device, void *data) {
  ...
  res = ata_io_start(data, 1, 0xec, 0, 0, 0, 0, (device << 4) & 0xffff, ATA_C_SCE_SEC_CONTROL);
  if (!res)
    return ata_io_finish();
  return res;
}
...
static int ata_device_smart_enable(int device) {
  ...
  res = ata_io_start(NULL, 1, ATA_C_SMART_ENABLE, 0, 0, 0x4f, 0xc2, (device << 4) & 0xffff, ATA_C_SMART);
  if (!res)
    return ata_io_finish();
  return res;
}
...
int ata_device_smart_save_attr(int device) {
  ...
  res = ata_io_start(NULL, 1, ATA_C_SMART_SAVE_ATTR, 0, 0, 0x4f, 0xc2, (device << 4) & 0xffff, ATA_C_SMART);
  if (!res)
    return ata_io_finish();
  return res;
}
...
int ata_device_sec_set_password(int device, void *password) {
  ...
  res = ata_io_start(param, 1, feature, 0, 0, 0, 0, (device << 4) & 0xffff, command);
  if (res == 0)
    res = ata_io_finish();
...
int ata_device_sec_unlock(int device, void *password) {
  ...
  if ((res = ata_io_start(param, 1, feature, 0, 0, 0, 0, (device << 4) & 0xffff, command)) != 0)
    return res;
  if ((res = ata_io_finish()) != 0)
    return res;
...
int ata_device_sec_erase(int device, void *password) {
  ...
  if ((res = ata_io_start(NULL, 1, feature, 0, 0, 0, 0, (device << 4) & 0xffff, command)) != 0)
    goto finish;
  if ((res = ata_io_finish()) != 0)
    goto finish;
...
/* Export 18 */
int ata_device_is_sce(int device) { return is_sony_hdd[device]; }
